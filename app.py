            ...
            with st.spinner(f"Extraindo dados e analisando {ticker_selecionado} (Buscando em cache se disponível)..."):
                ...
                if "erro" in relatorio:
                    ...
                    with col_ia:
                        ...
                        st.write(f"**Síntese:** {analise_noticias.get('sintese_geral', relatorio.get('sintese_geral', 'N/D'))}")
                        st.markdown("<br>", unsafe_allow_html=True)
                        
                        lista_noticias = analise_noticias.get('classificacao_individual', [])
                        
                        if lista_noticias:
                            # Mapeamento elegante de emojis em vez de if/elif
                            ...
                            for noti in lista_noticias:
                                if isinstance(noti, dict):
                                    ...
                                    emoji = emoji_map.get(sentimento, "⚪")
                        ...
                        for p in relatorio.get('perguntas_investigacao', []):
                            st.markdown(f"- {p}")
                    ...
                    with col_dados:
                        ...
                        with st.expander("📰 Últimas Notícias (RSS)", expanded=True):
                            if noticias and isinstance(noticias, list) and "aviso" not in noticias[0] and "erro" not in noticias[0]:
                                for noti in noticias:
                                    st.markdown(f"**{noti.get('titulo', 'Sem título')}**")
                                    st.caption(f"📅 *Publicado em: {noti.get('data', 'Data desconhecida')}*")
                                    if 'link' in noti:
...
# Executa o script apenas se ele for o arquivo principal
